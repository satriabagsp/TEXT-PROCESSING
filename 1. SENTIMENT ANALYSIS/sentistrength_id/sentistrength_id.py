...
class sentistrength:
	def __init__(self, config=dict()):
		...
		# Menyesuaikan konfigurasi yang telah diatur oleh user sebelumnya
	...
	# Fungsi klasifikasi hasil sentimen
	def classify(self):
		result = "neutral"
		try:
			if self.mean_conf:
				...
				if abs(self.sentences_max_pos) > abs(self.sentences_max_neg): # Positif jika nilai positif terbesar lebih besar dibanding nilai negatif terkecil
					result = "Positif"
				elif abs(self.sentences_max_pos) < abs(self.sentences_max_neg): # Negatif jika nilai positif terbesar lebih kecil dibanding nilai negatif terkecil
					result = "Negatif"
				elif abs(self.sentences_max_pos) == abs(self.sentences_max_neg): # Netral jika nilai positif terbesar sama dengan nilai negatif terkecil
					result = "Netral"
		except:
	...
	def main(self,sentence):
		...
		for sentence in sentences:
			...
			for i,term in enumerate(terms):
				...
				# Mengecek keberadaan kata negasi di sebelum kata yang sedang dinilai
				if self.negation_conf and self.score !=0 and i>0:
					self.cek_negationword(terms[i-1],terms[i-2])

				# Mengecek keberadaan kata booster di sesudah atau sebelum kata yang sedang dinilai
				if self.booster_conf and self.score !=0 and i>0 and i<=(terms_length-1):self.cek_boosterword(terms[i-1])
				if self.booster_conf and self.score !=0 and i>=0 and i<(terms_length-1):self.cek_boosterword(terms[i+1])

				# Mengecek apakah kata yang sedang dinilai termasuk idiom ketika digabung dengan kata sebelum atau sesudahnya
				if self.ungkapan_conf and i>0 and i<=(terms_length-1):self.cek_ungkapan([terms[i-1],term],[terms[i-2],terms[i-1],term],i)

				# Mengecek apakah terdapat pengulangan kata
				...
				# Mengecek keberadaan kata tanya
				...
				# Mendapatkan nilai positif atau nilai negatif dari kata yang sedang dinilai
				...
				# Memasukan nilai positif atau nilai negatif ke dalam variabel untuk dibandingkan nilainya dengan kata lain pada kalimat yang sama
